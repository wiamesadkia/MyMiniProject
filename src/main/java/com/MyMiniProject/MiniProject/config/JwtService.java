...
//    }
    ...
    /**
     * Check if the provided token has validated.
     *
     * @param token The JWT token to be checked.
     * @return True if the token has validated, otherwise false.
     */
    public boolean isTokenValid(String token) {
        boolean signed = Jwts.parserBuilder().setSigningKey(secretKey).build().isSigned(token);
        return signed && !isTokenExpired(token) ;
    }
